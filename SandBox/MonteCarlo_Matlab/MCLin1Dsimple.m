...
for m = 1:M %for each particle
    xtm1 = CHItm1(m,1); %get previous state
    xt = sample_motion_model(ut, xtm1,SIGproc); %update via motion model
    wt = measurement_model(zt, xt, SIGmeas); %obtain weigtht from measurement model 
    CHIbart(m,:) = [xt,wt]; %save current states and their weights
    N = N+wt; %sum weight for normalization later on
end

% normalize
cumwt = cumsum(CHIbart(:,2))/N; %normalize the new distribution to be a PDF
